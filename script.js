...
function initCanvas(config) {
  ...
  let mouse = {
    x: null,
    y: null,
    radius: (canvas.height / settings.mouseRadiusDivider) * (canvas.width / settings.mouseRadiusDivider)
  };
  ...
  function init() {
    particlesArray = [];
    const numberOfParticles = (canvas.height * canvas.width) / settings.countDivider;
  ...
  function connect() {
    ...
    for (let a = 0; a < particlesArray.length; a++) {
      for (let b = a; b < particlesArray.length; b++) {
        ...
        if (distance < (canvas.width / settings.connectionDivider) * (canvas.height / settings.connectionDivider)) {
...
function renderProfile(profile) {
  ...
  // Scramble on hover
  nameEl.addEventListener('mouseenter', () => scrambleText(nameEl, profile.name));
  ...
  document.getElementById('heroLocation').textContent = `${profile.location} • ${profile.status}`;
...
function renderExperience(experience) {
  ...
  experience.forEach(job => {
    ...
    const responsibilities = job.description.map(item => `<li>${item}</li>`).join('');
...
function setupCustomCursor() {
  ...
  const interactiveElements = document.querySelectorAll('a, button, .btn, .pill, .card');
...
function setupMagneticButtons() {
  const buttons = document.querySelectorAll('.btn, .pill, .contact-card, .quick-links a'); // Added contact-card and quick-links
